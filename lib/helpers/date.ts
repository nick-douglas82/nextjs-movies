...
export const formatLongMonthDate = (isoString?: string) => {
  if (!isoString) {
    return undefined;
  }

  return format(parseISO(isoString), "do MMMM yyyy");
};
...
export const formatRuntime = (runtime?: number) => {
  ...
  return formattedHours && formattedMinutes ? `${formattedHours} ${formattedMinutes}` : formattedHours ? formattedHours : formattedMinutes;
};
